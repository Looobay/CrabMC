...
This EULA is a legal agreement between you and us (Mojang AB and Microsoft Corporation, or, if applicable, one of its local affiliates listed in the Company Information section below).
You should read the whole thing but here is a quick summary of some important points to help guide you - the full terms and conditions still apply though.
...
We have helpful resources here (https://www.microsoft.com/online-safety/resources) that can help you be safe on the Internet.
...
"#.to_string();
    ...
    info!("Do you agree the EULA after reading it?\n(y/n):");
    ...
    match reponse.trim().to_lowercase().as_str() {
        "y" => {
            info!("You just agreed the EULA");

            // Delete eula
            if let Err(e) = fs::remove_file("eula.txt") {
        ...
        _ => {
            error!("This answer is not accepted (Use 'y' for yes or 'n' for no)");
            std::process::exit(0);
        }
    }
}
...
pub fn delete_eula() {
    ...
    match fs::remove_file(eula_path) {
        Ok(_) => info!("eula.txt deleted."),
        Err(e) => error!("Error when deleting eula.txt: {}", e),
    }
}
